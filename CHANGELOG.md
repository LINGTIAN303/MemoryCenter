...
### 改动
- `crates/memory-center-mcp/src/lib.rs`：
...
### 测试结果
- memory-center-mcp: 56 passed（原 51 + 新增 5），0 failed
...
- **`memory-center-mcp::bootstrap` 模块**：抽离 5 个启动期 `build_*` 函数供 stdio + HTTP 双入口复用
  - `build_conflict_detector` / `build_session_search` / `build_summary_generator`
  - `build_scenario_detector` / `build_combined_profile`
- **`memory-center-server::mcp` 模块**：MCP Streamable HTTP 路由挂载
  - `McpConfig::from_env()`：环境变量驱动配置
  - `mount_mcp_route()`：根据 stateful_mode 选择 SessionManager 并挂载到 Router
  - `make_service_factory()`：闭包内构造 MemoryCenterMcp 实例（每 session 一个）

### 改动
- `Cargo.toml`（workspace）：rmcp 1.8 启用 `transport-io` + `transport-streamable-http-server` features
- `crates/memory-center-mcp/src/lib.rs`：新增 `pub mod bootstrap;`
- `crates/memory-center-mcp/src/main.rs`：移除内联 build_*，改用 `bootstrap::build_*`（stdio 行为不变）
- `crates/memory-center-server/Cargo.toml`：新增 rmcp + memory-center-mcp + axum 依赖
- `crates/memory-center-server/src/lib.rs`：新增 `pub mod mcp;`
- `crates/memory-center-server/src/main.rs`：
  - 移除 4 个本地 build_* 函数定义（~150 行）
  - 改用 `bootstrap::build_*` 构造 AppState 组件
  - 新增 MCP 启用/禁用逻辑（`MEMORY_CENTER_MCP_ENABLED` 环境变量驱动，默认 false）
  - 启用时调用 `mount_mcp_route(app, &mcp_config)` 追加 `/mcp` 路由
- `crates/memory-center-server/src/mcp.rs`：新建文件（配置 + service_factory + 路由挂载）
...
| `MEMORY_CENTER_MCP_STATEFUL` | 是否启用 session 模式（true: LocalSessionManager / false: NeverSessionManager） | `true` |
...
- **Agent 识别限制**：rmcp `service_factory` 签名不支持传入 ClientInfo，HTTP 模式下 per-session 识别依赖 `MEMORY_CENTER_PRESET_AGENT` 环境变量（Layer 1），Layer 2 失效。生产环境推荐在 systemd unit 设置该变量
- **stdio bin 保留**：`memory-center-mcp` bin 仍为 stdio 模式入口，本地开发零配置

### 测试
- memory-center-server: 41 passed（含 mcp.rs doc test ignored）
- memory-center-mcp: 51 passed（lib）+ 5 doc tests ignored
- 总计 92 passed，0 failed
- cargo build 全量编译通过

### 向后兼容
- 未设置 `MEMORY_CENTER_MCP_ENABLED=true` 时，HTTP Server 行为与 v2.35 完全一致
...
- 新建 `memory-center-wasm` crate：wasm-bindgen 绑定 + MemoryStorage + JsStorage + MemoryCenterCore
...
- MemoryCenterCore JS API：archive / list_memories / read_memory / read_index
...
- WASM 编译验证通过（memory-center-core-logic + memory-center-wasm 均编译为 wasm32）
...
- `memory-center-presets::detect::tests::test_detect_from_explicit_env_valid` 在并行测试下偶发失败（env var race，与 WASM 改动无关，单线程模式下通过）
...
### 改动
- `crates/memory-center-core/src/model.rs`：IndexHook 新增 2 字段 + 修复 5 处构造点
- `crates/memory-center-core/src/storage.rs`：Storage trait + LocalStorage 实现（文件 `sessions/{sid}/raw_contexts/{hook_id}.txt`）
- `crates/memory-center-core/src/sqlite.rs`：SqliteStorage 实现 + `raw_contexts` 表 + 幂等迁移
- `crates/memory-center-core/src/cache.rs`：CachedStorage 透传
- `crates/memory-center-core/src/context_parser.rs`：新建模块（263 行）
- `crates/memory-center-core/src/lib.rs`：导出 context_parser 模块
- `crates/memory-center-mcp/src/lib.rs`：PreCompressParams/Result + pre_compress_hook 方法 + 辅助方法
- `crates/memory-center-mcp/Cargo.toml`：uuid 提升为生产依赖
- `crates/memory-center-mcp/tests/pre_compress_integration.rs`：新建，4 个集成测试
- `crates/memory-center-server/src/handlers.rs`：PreCompressRequest + pre_compress_handler
- `crates/memory-center-server/src/lib.rs`：路由注册
- `crates/memory-center-server/Cargo.toml`：新增 uuid + chrono 生产依赖
- `crates/memory-center-server/tests/http_integration.rs`：新增 3 个 HTTP 集成测试
- `AGENTS.md` + `.trae/rules/memory-center-archive.md`：调用规则更新
...
#### 背景
v2.30.1 完成了 archive 入参简化，但 Agent 客户端压缩上下文时 memory-center 无法感知被压缩的轮次。本版本通过"伪钩子方案"（外部反馈循环模拟主动感知）+ project_memory 反向写入，让 memory-center 记忆主动流入 Agent 客户端的 Memory Context。
...
- **治本**：`install_rules_to_project` 新增写入 AGENTS.md 逻辑
  ...
  - 所有客户端通用，用 `MEMORY_CENTER_AGENTS_BEGIN/END` 标记隔离
...
新增 `update_project_memory` + `get_project_memory` MCP 工具，让 memory-center 记忆"流入"第7层 Memory Context。

- 固定章节覆盖策略：用 `<!-- MEMORY_CENTER:SECTION:{name} START/END -->` 标记界定
...
#### 验证
- memory-center-core: 301 + 6 测试通过
- memory-center-server: 15 单元测试 + 37 集成测试全部通过（此前 1 失败 + 2 死锁，现在 37/37 全绿）
- memory-center-mcp: 46 测试通过

### v2.32 - 运行时配置查询工具 get_config（2026-07-06）

#### 背景
LLM 在调用 memory-center 工具时，无法感知当前服务的运行时配置（哪些 LLM 组件已注入、用什么模式、降级状态如何）。本版本新增 `get_config` 工具，让 LLM 主动查询运行时配置快照。

#### 变更
- **`RuntimeStatus` struct**（`crates/memory-center-mcp/src/lib.rs`）
...
- **HttpScenarioDetector**：LLM 兜底，复用 `LlmDetectorConfig`（MEMORY_CENTER_DETECTOR_* 环境变量），OpenAI 兼容 API
...
#### 变更
- `crates/memory-center-core/src/storage.rs`：SessionMeta struct + Storage trait 2 新方法 + LocalStorage 实现
- `crates/memory-center-core/src/sqlite.rs`：SqliteStorage session_meta 表 + 2 方法
- `crates/memory-center-core/src/cache.rs`：CachedStorage 透传
- `crates/memory-center-presets/src/builder.rs`：scenario_to_str + scenario_from_str 支持 custom: 前缀
- `crates/memory-center-presets/src/scenario_detect.rs`（新文件）：3 Detector + resolve_effective_scenario + 28 测试
- `crates/memory-center-mcp/src/lib.rs`：MemoryCenterMcp 注入 scenario_detector + archive handler 调用 resolve_effective_scenario
- `crates/memory-center-mcp/src/main.rs`：build_scenario_detector 函数
- `crates/memory-center-server/src/lib.rs`：AppState 新增 scenario_detector 字段
- `crates/memory-center-server/src/main.rs`：build_scenario_detector 函数
- `crates/memory-center-server/src/handlers.rs`：archive handler 接入 resolve_effective_scenario

#### 验证
- 生产环境验证：传入写作类对话 archive，关键词模式识别为 "writing" 场景，置信度 0.9，正确写入 session_meta
- 工作区全测试通过（800+ 测试）
- clippy 无 warning（memory-center-server 范围）
...
- **DRY 公共函数**：`memory_center_presets::build_from_strings` 被 server / mcp / python 三端共享

#### 变更

##### 阶段 1：core 改造
- **`SummaryGenerator` trait**（`crates/memory-center-core/src/generate.rs`）
  - 新增默认方法 `generate_summary_with_template(file, template)`，默认实现忽略 template 调用 `generate_summary`（向后兼容）
- **`Archiver` / `Compactor`**（`crates/memory-center-core/src/archive.rs` / `compact.rs`）
...
- **`HttpSummaryGenerator`**（`crates/memory-center-llm/src/summary_generator.rs`）
  - 覆盖 `generate_summary_with_template`，提取公共 `call_llm` 方法

##### 阶段 2：HTTP API
- **`crates/memory-center-server/src/presets.rs`**（新文件）
...
- **`crates/memory-center-server/src/handlers.rs`**
...
##### 阶段 3：MCP 工具
- **`crates/memory-center-presets/src/builder.rs`**
...
##### 阶段 4：Python 绑定
- **`crates/memory-center-python/Cargo.toml`**：新增 3 个依赖（memory-center-models / -windows / -skills）
- **`crates/memory-center-python/src/lib.rs`**
  - 删除本地 `scenario_from_str`，改用 `memory_center_presets::scenario_from_str`（DRY）
  ...
  - `MemoryCenter.archive()` 新增可选 `preset` 参数（`#[pyo3(signature = (turns, preset=None))]`）
...
#### 测试
- memory-center-python：19 测试通过（含 10 个新增）
- memory-center-mcp：38 测试通过（含 13 个新增）
- memory-center-presets：21 测试通过（含 2 个新增公共函数测试）
...
- **Python**：`MemoryCenter.archive(turns, preset=None)` 默认 `None` 保持原行为
...
- `crates/memory-center-llm/src/summary_generator.rs`
- `crates/memory-center-presets/src/builder.rs`
- `crates/memory-center-presets/src/lib.rs`
- `crates/memory-center-server/Cargo.toml`
- `crates/memory-center-server/src/presets.rs`（新）
- `crates/memory-center-server/src/lib.rs`
- `crates/memory-center-server/src/handlers.rs`
- `crates/memory-center-mcp/Cargo.toml`
- `crates/memory-center-mcp/src/lib.rs`
- `crates/memory-center-python/Cargo.toml`
- `crates/memory-center-python/src/lib.rs`
...
#### 变更
- **`HybridDetector::detect`**（`crates/memory-center-core/src/conflict.rs`）
...
#### 修复
- **`update_memory` key_facts 注入**（`crates/memory-center-server/src/handlers.rs`）
...
- **本地 production remote**：`__REDACTED_SERVER__:/root/memory-center.git`
...
- **v2.24: 关闭 LLM 思考模式**
  - 3 个 LLM 客户端请求体加 `"thinking": {"type": "disabled"}`
    - `crates/memory-center-llm/src/detector.rs`
    - `crates/memory-center-llm/src/scorer.rs`
    - `crates/memory-center-llm/src/summary_generator.rs`
...
#### 新增
- **API Key 鉴权中间件**（`crates/memory-center-server/src/middleware/auth.rs`）
  - 从 `Authorization: Bearer <key>` 头提取 API Key 比对
  - 环境变量 `MEMORY_CENTER_API_KEY` 驱动，未配置时跳过鉴权（向后兼容）
  - 常量时间比对（避免时序侧信道攻击）
...
- **Nginx 配置示例**（`deploy/nginx-memory-center.conf` + `deploy/nginx-memory-center-block.conf`）

#### 变更
- `lib.rs` create_router 应用 `middleware::from_fn(require_api_key)` 到所有路由
- `main.rs` 启动时打印 API Key 鉴权状态（已启用/未启用警告）
- 中间件模块独立成 `crates/memory-center-server/src/middleware/`（遵循工程规范第 5 条）

#### 测试
- memory-center-server：5 lib + 44 集成 = 49 测试全通过
- 服务器 E2E：归档/检索/摘要/Prompt/公网反代 5 项 200 OK

#### 部署验证
- 服务器：162.211.183.236（openworld.dpdns.org）
- 二进制：/opt/memory-center-server/bin/memory-center-server（9.2MB）
- systemd：memory-center-server.service（active running, Restart=always）
- Nginx：`/memory-center/` 子路径反代到 127.0.0.1:8765
- 公网入口：https://openworld.dpdns.org/memory-center/api/v1/...
...
- stdio 传输入口（main.rs），通过环境变量 `MEMORY_CENTER_ROOT` 配置存储根目录
...
- workspace members 新增 `crates/memory-center-mcp`
...
- 环境变量配置：`MEMORY_CENTER_HOST`（默认 127.0.0.1）/ `MEMORY_CENTER_PORT`（默认 8765）/ `MEMORY_CENTER_ROOT`（默认 ./data）
- 14 个 HTTP 集成测试（reqwest 客户端 + 随机端口 TestServer）
- CI 新增 `http-integration-test` job

#### v2.2 - Python 原生绑定（commit a3ed611）
- 新增 `memory-center-python` crate（PyO3 0.29 + maturin build backend，cdylib）
- `MemoryCenter` pyclass：OOP 风格 + `__enter__`/`__exit__` 上下文管理器 + `__repr__`
...
#### P4 - C ABI 动态库
- 5 个核心 C ABI 操作：
  - `memory_center_archive`：归档一批轮次，返回 SummaryView JSON
  - `memory_center_retrieve`：按钩子 ID 检索完整记忆文件
  - `memory_center_get_summaries`：获取所有周期摘要视图
  - `memory_center_render_prompt`：渲染摘要为 system prompt 文本
  - `memory_center_run_compaction`：触发周期任务（周合并/月淘汰）
- 内部 tokio Runtime（current_thread，block_on 异步 Core 方法）
- C 头文件 `memory_center.h` 完整定义
