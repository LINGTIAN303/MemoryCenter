...
use hippocampus::*; // crate lib name = "hippocampus"
use std::ffi::CStr;
use std::ptr;
...
fn make_handle(dir: &TempDir, session: &str) -> *mut HippocampusHandle {
    let root = dir.path().to_str().unwrap();
    let root_c = CString::new(root).unwrap();
    let session_c = CString::new(session).unwrap();
    unsafe { hippocampus_new(root_c.as_ptr(), session_c.as_ptr(), ptr::null()) }
}

/// 从结果获取数据字符串（负责释放）
unsafe fn get_data(result: *const HippocampusResult) -> String {
    let data_ptr = hippocampus_get_data(result);
    if data_ptr.is_null() {
        return String::new();
    }
...
/// 从结果获取错误字符串（负责释放）
unsafe fn get_error(result: *const HippocampusResult) -> String {
    let err_ptr = hippocampus_get_error(result);
    if err_ptr.is_null() {
        return String::new();
    }
    let s = CStr::from_ptr(err_ptr).to_string_lossy().to_string();
    hippocampus_free_string(err_ptr);
    s
}

/// 归档一批 turns 并返回 hook_id
fn archive_turns(handle: *mut HippocampusHandle, turns: Vec<MessageTurn>) -> String {
    let turns_json = serde_json::to_string(&turns).unwrap();
    let turns_c = CString::new(turns_json).unwrap();
    let result = unsafe { hippocampus_archive(handle, turns_c.as_ptr()) };
    assert!(unsafe { hippocampus_is_ok(result) }, "归档失败");
    let data = unsafe { get_data(result) };
    unsafe { hippocampus_result_free(result) };
...
fn test_handle_lifecycle() {
    ...
    unsafe { hippocampus_free(handle) };
}
...
fn test_handle_null_params() {
    unsafe {
        ...
        assert!(hippocampus_new(ptr::null(), session_c.as_ptr(), ptr::null()).is_null());
        // session_id 为 NULL
        let root_c = CString::new("/tmp").unwrap();
        assert!(hippocampus_new(root_c.as_ptr(), ptr::null(), ptr::null()).is_null());
    }
}
...
fn test_archive_and_retrieve() {
    ...
    let result = unsafe { hippocampus_retrieve(handle, hook_id_c.as_ptr()) };
    assert!(unsafe { hippocampus_is_ok(result) });
...
fn test_retrieve_invalid_hook_id() {
    ...
    let result = unsafe { hippocampus_retrieve(handle, invalid_id.as_ptr()) };
    // 无效 hook_id 应返回失败
    assert!(!unsafe { hippocampus_is_ok(result) });
...
fn test_get_summaries_empty() {
    ...
    let result = unsafe { hippocampus_get_summaries(handle) };
    assert!(unsafe { hippocampus_is_ok(result) });
...
fn test_render_prompt_empty() {
    ...
    let result = unsafe { hippocampus_render_prompt(handle) };
    assert!(unsafe { hippocampus_is_ok(result) });
...
fn test_archive_invalid_json() {
    ...
    let result = unsafe { hippocampus_archive(handle, bad_c.as_ptr()) };
    assert!(!unsafe { hippocampus_is_ok(result) });
...
fn test_null_handle_returns_error() {
    unsafe {
        let turns_c = CString::new("[]").unwrap();
        let result = hippocampus_archive(ptr::null_mut(), turns_c.as_ptr());
        assert!(!hippocampus_is_ok(result));
        hippocampus_result_free(result);

        let result = hippocampus_get_summaries(ptr::null_mut());
        assert!(!hippocampus_is_ok(result));
        hippocampus_result_free(result);
    }
}
...
fn test_run_compaction_invalid_period() {
    ...
    let result = unsafe { hippocampus_run_compaction(handle, 99) };
    assert!(!unsafe { hippocampus_is_ok(result) });
...
fn test_run_compaction_weekly_no_daily() {
    ...
    let result = unsafe { hippocampus_run_compaction(handle, COMPACTION_WEEKLY) };
    assert!(!unsafe { hippocampus_is_ok(result) });
...
fn test_full_workflow_archive_then_compact() {
    ...
    // 2. 验证摘要
    let summaries_result = unsafe { hippocampus_get_summaries(handle) };
    assert!(unsafe { hippocampus_is_ok(summaries_result) });
    ...
    unsafe { hippocampus_result_free(summaries_result) };

    // 3. 周级合并
    let compact_result = unsafe { hippocampus_run_compaction(handle, COMPACTION_WEEKLY) };
    assert!(unsafe { hippocampus_is_ok(compact_result) });
    ...
    unsafe { hippocampus_result_free(compact_result) };
...
fn test_result_free_idempotent() {
    // 释放 NULL 结果不应崩溃
    unsafe { hippocampus_result_free(ptr::null_mut()) };
    // 释放 NULL 字符串不应崩溃
    unsafe { hippocampus_free_string(ptr::null_mut()) };
    // 释放 NULL handle 不应崩溃
    unsafe { hippocampus_free(ptr::null_mut()) };
}

#[test]
fn test_get_data_on_null_result() {
    unsafe {
        assert!(hippocampus_get_data(ptr::null()).is_null());
        assert!(hippocampus_get_error(ptr::null()).is_null());
        assert!(!hippocampus_is_ok(ptr::null()));
    }
}
