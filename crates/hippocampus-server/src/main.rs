...
//! 与 stdio 模式共享同一套 `build_*` 组件构造逻辑（`hippocampus_mcp::bootstrap`）。
//!
//! | 环境变量 | 说明 | 默认值 |
//! |---------|------|--------|
...
// v2.36：复用 hippocampus-mcp::bootstrap 的 build_* 函数（与 stdio 模式一致）
...
use hippocampus_server::mcp::{mount_mcp_route, McpConfig};
...
async fn main() {
    ...
    let app = if std::env::var("HIPPOCAMPUS_MCP_ENABLED")
        .ok()
        ...
        .unwrap_or(false)
    {
        ...
        tracing::info!("MCP Streamable HTTP 端点未启用（设置 HIPPOCAMPUS_MCP_ENABLED=true 启用）");
        app
    };
    ...
    tracing::info!("  POST/GET/DELETE  /mcp  (v2.36 MCP Streamable HTTP，需启用 HIPPOCAMPUS_MCP_ENABLED)");
