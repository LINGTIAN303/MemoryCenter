//! # MemoryCenter Agent 代理工具特配库
//!
//! 5 种特配 crate 之一（平行拓扑，仅依赖 MemoryCenter-core）。
//!
//! ## 定位
//!
//! 识别当前使用 MemoryCenter 的 Agent 代理工具（Claude Code / Cursor / Trae 等），
//! 为 presets 组合层提供 agent 维度的默认配置：
//! - 是否归档到 MemoryCenter（部分 Agent 有自己的记忆机制）
//! - 是否支持工具调用（影响标签策略）
...
//! Agent → 压缩方式 / 场景 / 技能的映射由 `MemoryCenter-presets` 组合层处理。
//!
//! ## 联动机制（由 presets 层实现）
//!
...
pub use hook_mode::{HookMode, HookModeResolver};
