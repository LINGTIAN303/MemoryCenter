...
/// LLM 调用 archive 时可传入此快照，MemoryCenter 持久化到 session_state.json。
...
/// ## 与 Trae Summary 的关系
///
/// - MemoryCenter 记忆 = 一手源数据（LLM 主动传入）
/// - Trae Summary 第8章节 = 二手信息（Trae 客户端生成）
/// - 两者冲突时，以 MemoryCenter 快照为准（详见「压缩后行为协议」）
#[derive(Debug, Clone, Serialize, Deserialize)]
