//! # LLM 冲突检测器（v2.10）
//!
//! 基于 [`memory_center_core::conflict::ConflictDetector`] trait 的 HTTP 实现，
...
use memory_center_core::conflict::{
    ConflictDetector, ConflictKind, ConflictRecord, ConflictReport, Severity,
};
use memory_center_core::model::{MemoryFile, MemoryUpdate};
...
impl LlmDetectorConfig {
    /// 环境变量前缀
    pub const ENV_PREFIX: &'static str = "MEMORY_CENTER_DETECTOR";

    /// 从环境变量构造配置（v2.13 新增）
    ///
    /// 读取以下环境变量（前缀 `MEMORY_CENTER_DETECTOR_`）：
    ///
    /// | 环境变量 | 字段 | 默认值 |
    /// |---------|------|--------|
