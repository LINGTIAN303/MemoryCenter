...
//!
//! 基于 [`memory_center_core::generate::SummaryGenerator`] trait 的 HTTP 实现，
...
use memory_center_core::generate::{LlmGeneratorConfig, SummaryGenerator};
use memory_center_core::model::{MemoryFile, Summary, Tag};
...
pub struct HttpSummaryGenerator {
    ...
    ///
    /// 由调用方（memory-center-server / mcp）通过 CombinedProfile 解析后注入。
...
impl HttpSummaryGenerator {
    ...
    /// ```
    ///
    /// 调用方（memory-center-server / mcp）通过 `CombinedProfile::summary_template()`
    /// 获取最终模板后注入本生成器。
...
impl SummaryGenerator for HttpSummaryGenerator {
    async fn generate_summary(&self, file: &MemoryFile) -> memory_center_core::Result<Summary> {
        // 未配置 API URL：降级为启发式
...
mod tests {
    ...
    fn test_parse_json_summary() {
        let json = r#"{"title":"三级索引设计","abstract":"讨论了记忆库架构","key_facts":["采用三级周期"],"key_entities":["Rust","MemoryCenter"]}"#;
