...
//!
//! 将 MemoryCenter 核心能力暴露为 MCP (Model Context Protocol) tools，
...
//! - 未来可扩展 Streamable HTTP（挂载到 memory-center-server 的 axum Router）
//!
//! ## 使用示例
//!
...
// 供 memory-center-mcp bin（stdio）和 memory-center-server bin（HTTP）复用
pub mod bootstrap;
...
impl MemoryCenterMcp {
    ...
    ///
    /// ```rust,ignore
    /// use memory_center_presets::{detect_agent_client, resolve_scenario_name, PresetBuilder};
...
struct ArchiveParams {
    ...
    ///
    /// LLM 主动传入当前任务状态，MemoryCenter 持久化到 session_state.json。
...
///
/// 与 memory_center_core::model::TaskStateSnapshot 对应，
...
/// pre_compress_hook tool 参数（v2.34 新增）
///
...
pub struct PreCompressParams {
    ...
    session_id: String,
    /// 完整上下文字符串
    #[schemars(description = "完整上下文字符串。客户端 dump 整个对话或 LLM 拼接关键内容。支持 JSON 数组([{user_message,llm_message}]) 或 User:/Assistant: 分隔符格式，无法识别时仅存 raw_context 不阻塞")]
    full_context: String,
    /// 客户端估算的原始 token 数（可选）
    #[schemars(description = "客户端估算的原始 token 数，用于反馈循环。未传时服务端按 full_context.len()/3 估算")]
    #[serde(default)]
    estimated_tokens: Option<usize>,
    /// 预设配置（可选，与 archive 的 PresetParams 一致）
    #[schemars(description = "预设配置（可选，传入后应用 archive_threshold + summary_template 覆盖默认行为）")]
    #[serde(default)]
    preset: Option<PresetParams>,
    /// 任务状态快照（可选，与 archive 的 task_state_snapshot 一致）
    #[schemars(description = "任务状态快照（可选）。传入后持久化到 session_state.json，下次 prompt 时返回。用于压缩后校准 Trae Summary 第8章节 Current Work")]
    #[serde(default)]
    task_state_snapshot: Option<TaskStateSnapshotParams>,
    /// 项目 ID（可选）
    #[schemars(description = "项目 ID（可选，用于项目级隔离）")]
...
/// pre_compress_hook 返回结果（v2.34 新增）
///
/// 与 archive 的 JSON 返回结构对等，额外包含 `raw_context_path` /
/// `parse_success` / `parsed_turns_count` 三个 pre_compress 特有字段。
///
/// 序列化为 JSON 字符串后通过 MCP tool 返回（与 archive 一致）。
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, schemars::JsonSchema)]
pub struct PreCompressResult {
    /// 钩子 ID（UUID v4，用于后续 retrieve / raw_context 检索）
    pub hook_id: String,
    /// raw_context 文件相对路径（如 sessions/{sid}/raw_contexts/{hook_id}.txt）
    pub raw_context_path: String,
    /// 是否成功解析 turns（false 表示仅存 raw_context，未走 Archiver）
    pub parse_success: bool,
    /// 解析出的轮次数（0 表示未解析或解析失败）
    pub parsed_turns_count: usize,
    /// 归档的 token 数（parse_success=true 时为 Archiver 估算；false 时等于 estimated_total_tokens）
    pub archived_tokens: usize,
    /// 估算的累计 token 数（客户端传入或服务端按 len/3 估算）
    pub estimated_total_tokens: usize,
    /// 当前 preset 的归档阈值（token 数）
    pub threshold: usize,
    /// 当前占比百分比（archived_tokens / threshold * 100，四舍五入）
    pub threshold_ratio_percent: u64,
    /// 人类可读建议（含 parse_success 状态 + token 反馈）
    pub suggestion: String,
    /// 归档时间（ISO 8601 / RFC 3339）
...
struct InstallRulesParams {
    ...
    /// 是否强制覆盖（默认 false）
    #[serde(default)]
    #[schemars(description = "是否强制覆盖已存在的文件（默认 false：catpaw/trae 跳过已存在文件，claude-code 跳过已有 MemoryCenter 标记的文件）")]
    force: bool,
}
...
///
/// LLM 主动调用，用固定章节覆盖策略更新 MemoryCenter 维护的 project_memory.md 副本。
...
impl MemoryCenterMcp {
    ...
    #[tool(description = "渲染记忆摘要为 system prompt 文本。【会话开始必调】每个新会话的第一次回复前必须调用此工具，返回的历史记忆摘要拼接到 system prompt 末尾，让你了解之前讨论过什么。若返回空列表说明该 session 无历史记忆，正常继续。这是 MemoryCenter 记忆协议的第一步，无需用户提示主动执行。若该 session 存在 task_state_snapshot（archive 时传入），会在 prompt 末尾追加任务状态快照，用于压缩后校准 Trae Summary 第8章节 Current Work。")]
    async fn prompt(
        &self,
    ...
    async fn find_hook_by_prefix(
        ...
        let storage = self.create_storage();
        let matches = memory_center_core::retrieve::find_hooks_by_prefix(
            &storage,
            &params.hook_prefix,
            params.project_id.as_deref(),
    ...
    async fn batch_update(
        ...
        for pair in &mut pairs {
            ...
            let update = memory_center_core::model::MemoryUpdate::new()
        ...
        let session_search = match &self.session_search {
            ...
            None => {
                return Err(McpError::internal_error(
                    "semantic_search 工具未启用：未注入 SessionSearchRouter（需在 MCP 启动时配置 MEMORY_CENTER_EMBEDDER_* 环境变量）",
                    None,
                ));
            }
        };
    ...
    async fn preset_list_agents(
        ...
        let agents: Vec<_> = memory_center_agents::AgentFamily::all_builtin()
            .into_iter()
    ...
    async fn preset_list_scenarios(
        ...
        let scenarios: Vec<_> = memory_center_scenarios::Scenario::all_builtin()
            .iter()
            .map(|s| {
    ...
    async fn preset_list_models(
        ...
        let models: Vec<_> = memory_center_models::ModelRegistry::all_variants()
            .map(|(name, variant)| {
                let default = memory_center_models::ModelRegistry::default_variant(variant.family);
                serde_json::json!({
                    "name": name,
    ...
    ///
    /// 用固定章节覆盖策略更新 MemoryCenter 维护的 project_memory.md 副本。
    /// 章节用 HTML 注释标记界定（`<!-- MemoryCenter:SECTION:{name} START/END -->`），
    /// 不影响用户手动写入的内容。
    #[tool(description = "更新 project_memory.md 副本的指定章节。【让 MemoryCenter 记忆流入第7层 Memory Context】调用此工具后，MemoryCenter 维护的 project_memory.md 副本会被更新，返回 full_content 供你用 Write 工具写入 Trae 客户端的 memory 文件夹（如 c:\\Users\\<user>\\.trae-cn\\memory\\projects\\<project>\\project_memory.md）。\
     \
    ...
    async fn update_project_memory(
        ...
        let start_marker = format!("<!-- MemoryCenter:SECTION:{} START -->", section);
        let end_marker = format!("<!-- MemoryCenter:SECTION:{} END -->", section);
    ...
    #[tool(description = "读取 project_memory.md 副本完整内容。返回 MemoryCenter 维护的 project_memory.md 副本的完整 Markdown 内容。用于：(1)查看当前 MemoryCenter 写入了哪些章节；(2)拿到完整内容后用 Write 工具写入 Trae 客户端的 memory 文件夹。若文件不存在返回空字符串。")]
    ...
    // ========================================================================
    // v2.34：pre_compress_hook 工具（压缩前一次性完整归档）
    // ========================================================================
    //
    // 与 archive 互补：
    // - archive：日常归档，输入结构化 turns_json
    // - pre_compress_hook：压缩前一次性完整归档，输入 full_context 字符串
    //
    // 双轨处理（spec 第五章）：
    ...
    /// 压缩前一次性完整归档（v2.34 新增）。
    ///
    /// 当 LLM 感知到即将被压缩时调用（伪钩子增强）：
    /// - 用户明确说"压缩上下文" / "上下文好长"
    /// - 上次 archive 返回的 threshold_ratio_percent >= 90
    /// - 长任务执行中预判上下文即将超限
    ///
    /// 与 archive 的区别：接收完整上下文字符串而非结构化 turns，双轨存储
    ...
    pub async fn pre_compress_hook(
        ...
        let result_str = serde_json::to_string(&result).map_err(|e| {
            McpError::internal_error(format!("序列化 PreCompressResult 失败: {e}"), None)
        })?;
...
// ============================================================================
//
// 首次接入 MemoryCenter 时，将 Rules 模板写入 Agent 客户端的 rules 目录。
// 覆盖策略按客户端类型自动决定：
// - catpaw/trae（独立文件）：已存在则跳过（force=true 时覆盖）
// - claude-code（CLAUDE.md）：检查 MemoryCenter 标记，有标记则跳过/替换，无标记则追加
//
// 风险防护：
...
name: "MemoryCenter 记忆归档触发"
description: "当对话变长、上下文接近阈值、用户提到历史事件、即将压缩上下文时，主动调用 memory_center_mcp.archive 归档对话轮次。归档后返回 token 估算反馈，用于判断后续何时再次归档。这是伪钩子方案的核心：通过外部反馈建立 token 意识。"
globs:
---

# MemoryCenter 记忆归档触发规则

## 你的角色

你是接入了 MemoryCenter 记忆库的 Agent。MemoryCenter 帮你保留长对话中的关键信息，
避免上下文压缩时丢失重要内容。**你无法直接感知自身 token 消耗**，但通过 MemoryCenter
...
当你调用过 memory_center_mcp.archive 后，返回结果包含：
...
const TRAE_RULES_TEMPLATE: &str = r#"# MemoryCenter 记忆归档触发规则（Trae）
...
- **建议**：在 Trae 提示压缩前，先调用 `MemoryCenter.archive` 归档当前对话
- 避免压缩丢失原始内容
...
const CLAUDE_CODE_RULES_TEMPLATE: &str = r#"## MemoryCenter 记忆协议

本项目已接入 MemoryCenter 记忆库。Agent 必须遵守以下规则，无需用户提示主动执行。
...
```
MemoryCenter.prompt(session_id)
```
...
```
MemoryCenter.semantic_search(query, session_id, top_k=5)
```
...
```
MemoryCenter.detect_conflicts(session_id, hook_id, added_facts, revised_facts, deprecated_facts)
```
...
const MEMORY_CENTER_AGENTS_BEGIN: &str = "<!-- memory-center-agents begin -->";
const MEMORY_CENTER_AGENTS_END: &str = "<!-- memory-center-agents end -->";
...
const AGENTS_MD_TEMPLATE: &str = r#"<!-- memory-center-agents begin -->
# MemoryCenter 记忆协议（项目级硬性规则）

> 本文件由 install_rules 自动写入，被 IDE 自动读取注入 LLM system prompt。
> 完整规则详见 `.trae/rules/memory-center-archive.md`（或 `.catpaw/rules/`）。
...
### 1. 会话开始：调 prompt 获取历史记忆
每个新会话的第一次回复前，必须先调用 `mcp_MemoryCenter.prompt(session_id)`。
...
立即执行 4 步强制流程（详见 rules/memory-center-archive.md）：
1. 归档压缩前未持久化的轮次
2. 调 prompt 拉取 MemoryCenter 一手记忆
...
fn build_remote_template_response(client: &str, project_root: &str) -> Result<String, String> {
    ...
    let agents_md_content = format!(
        "{begin}\n{template}\n{end}",
        begin = MEMORY_CENTER_AGENTS_BEGIN,
        template = AGENTS_MD_TEMPLATE,
        end = MEMORY_CENTER_AGENTS_END,
    );
    ...
    let rules_content = if client == "claude-code" {
        format!(
            "{begin}\n{template}\n{end}",
            begin = MEMORY_CENTER_RULES_BEGIN,
            template = rules_template,
            end = MEMORY_CENTER_RULES_END,
        )
    } else {
        ...
        "files": [
            {
                ...
                "description": " MemoryCenter 记忆归档规则模板",
            },
            {
...
pub fn install_rules_to_project(
        ...
        "claude-code" => {
            ...
            if !file_path.exists() {
                fs::write(&file_path, &content_with_markers)
                    .map_err(|e| format!("写入失败: {e}"))?;
                let msg = "已创建 CLAUDE.md 并写入 MemoryCenter 规则".to_string();
                (file_path, "created", msg)
            } else {
                let existing = fs::read_to_string(&file_path)
                    .map_err(|e| format!("读取 CLAUDE.md 失败: {e}"))?;

                if existing.contains(MEMORY_CENTER_RULES_BEGIN) {
                    if !force {
                        let msg = "CLAUDE.md 已有 MemoryCenter 规则，跳过（force=false）".to_string();
                        (file_path, "skipped", msg)
                    } else {
                        let start_idx = existing.find(MEMORY_CENTER_RULES_BEGIN)
                            .ok_or("找不到开始标记")?;
                        let end_idx = existing.find(MEMORY_CENTER_RULES_END)
                            .ok_or("找不到结束标记")?;
                        let end_idx = end_idx + MEMORY_CENTER_RULES_END.len();
                        ...
                        fs::write(&file_path, new_content)
                            .map_err(|e| format!("写入失败: {e}"))?;
                        let msg = "已更新 CLAUDE.md 中的 MemoryCenter 规则".to_string();
                    ...
                    fs::write(&file_path, new_content)
                        .map_err(|e| format!("写入失败: {e}"))?;
                    let msg = "已追加 MemoryCenter 规则到 CLAUDE.md".to_string();
                    (file_path, "updated", msg)
                }
            }
        }
    ...
    let agents_action = if !agents_md_path.exists() {
        ...
        if existing.contains(MEMORY_CENTER_AGENTS_BEGIN) {
            // 已有 MemoryCenter 标记
            if !force {
                "skipped"
            } else {
                // 替换标记内容
                let start_idx = existing.find(MEMORY_CENTER_AGENTS_BEGIN)
                    .ok_or("AGENTS.md 找不到开始标记")?;
                let end_idx = existing.find(MEMORY_CENTER_AGENTS_END)
                    .ok_or("AGENTS.md 找不到结束标记")?;
                let end_idx = end_idx + MEMORY_CENTER_AGENTS_END.len();
                let new_content = format!(
                    "{}{}{}",
                    &existing[..start_idx],
...
#[tool_handler]
impl ServerHandler for MemoryCenterMcp {
    ...
    /// 在 memory-center-mcp 上下文展开 `env!`，让客户端看到正确的 "memory-center-mcp"。
    fn get_info(&self) -> ServerInfo {
        let capabilities = ServerCapabilities::builder()
            .enable_tools()
            .build();

        // 基础 ServerInfo（name/version 在 memory-center-mcp 上下文取，修正 v2.29 的 "rmcp" 问题）
        // 用 CARGO_PKG_NAME（kebab-case "memory-center-mcp"）而非 CARGO_CRATE_NAME（snake_case）
...
mod tests {
    ...
    /// 创建一个绑定到临时目录的 MCP 实例
    fn make_mcp(tmpdir: &TempDir) -> MemoryCenterMcp {
        MemoryCenterMcp::new(tmpdir.path().to_path_buf())
    }

    /// 创建一个注入 HeuristicDetector 的 MCP 实例（v2.11 测试用）
    fn make_mcp_with_detector(tmpdir: &TempDir) -> MemoryCenterMcp {
        let detector: Arc<dyn ConflictDetector> =
            Arc::new(memory_center_core::heuristic::HeuristicDetector::new());
        MemoryCenterMcp::with_conflict_detector(
    ...
    fn make_mcp_with_session_search(tmpdir: &TempDir) -> MemoryCenterMcp {
        use memory_center_search::SessionSearchRouter;
        let storage: Arc<dyn memory_center_core::storage::Storage> =
            Arc::new(memory_center_core::storage::LocalStorage::new(
                tmpdir.path().to_path_buf(),
            ));
        let router = SessionSearchRouter::new(None, 0).with_storage(storage);
        MemoryCenterMcp::with_conflict_detector(tmpdir.path().to_path_buf(), None)
            .with_session_search(Some(Arc::new(router)))
    }
    ...
    fn make_mcp_with_summary_generator(tmpdir: &TempDir, fail: bool) -> MemoryCenterMcp {
        ...
        impl SummaryGenerator for MockSummaryGenerator {
            async fn generate_summary(
                &self,
                _file: &MemoryFile,
            ) -> memory_center_core::Result<Summary> {
                if self.fail {
                    return Err(memory_center_core::Error::Storage(
                        "Mock 摘要生成失败".into(),
                    ));
                }
                Ok(Summary {
    ...
    #[test]
    fn test_construct() {
        let mcp = MemoryCenterMcp::new(PathBuf::from("/tmp/test"));
    ...
    async fn test_batch_update_without_detector_no_conflicts_field_default() {
        // 未注入检测器（MemoryCenterMcp::new）：batch_update 不检测冲突，
    ...
    fn make_mcp_with_full_profile(tmpdir: &TempDir) -> MemoryCenterMcp {
    ...
    fn test_combined_profile_default_none() {
        let tmpdir = TempDir::new().unwrap();
        let mcp = MemoryCenterMcp::new(tmpdir.path().to_path_buf());
        assert!(
            mcp.combined_profile().is_none(),
            "new() 默认不应注入 combined_profile"
        );

        let mcp2 = MemoryCenterMcp::with_conflict_detector(tmpdir.path().to_path_buf(), None);
        assert!(
    ...
    fn test_install_rules_remote_mode_returns_template() {
        ...
        // 第一个文件：.trae/rules/memory-center-archive.md
        assert_eq!(files[0]["relative_path"], ".trae/rules/memory-center-archive.md");
        assert_eq!(files[0]["mode"], "create");
        assert!(files[0]["content"].as_str().unwrap().contains("MemoryCenter"), "content 应包含模板内容");

        // 第二个文件：AGENTS.md
        assert_eq!(files[1]["relative_path"], "AGENTS.md");
        assert_eq!(files[1]["mode"], "append_with_markers");
        assert!(files[1]["content"].as_str().unwrap().contains("memory-center-agents begin"));
    }
    ...
    fn test_install_rules_remote_mode_catpaw() {
        ...
        let files = json["files"].as_array().unwrap();
        assert_eq!(files[0]["relative_path"], ".catpaw/rules/memory-center-archive.md");
    }
    ...
    fn test_install_rules_remote_mode_claude_code() {
        ...
        // claude-code 的 content 应包含 memory-center-rules begin 标记
        assert!(files[0]["content"].as_str().unwrap().contains("memory-center-rules begin"));
    }
    ...
    fn test_install_rules_local_mode_still_works() {
        ...
        // 验证文件确实被创建
        let rules_file = tmpdir.path().join(".trae").join("rules").join("memory-center-archive.md");
    ...
    fn test_with_combined_profile_injection() {
        use memory_center_agents::{AgentFamily, AgentProfile};
        use memory_center_presets::{PresetBuilder, CombinedProfile};
        use memory_center_scenarios::{Scenario, ScenarioProfile};
    ...
    fn test_detection_to_injection_full_chain_trae() {
        use memory_center_presets::{detect_agent_client, resolve_scenario_name, scenario_from_str, PresetBuilder};
        use memory_center_agents::AgentProfile;
        use memory_center_scenarios::ScenarioProfile;
    ...
    fn test_get_info_injects_instructions_when_combined_profile_present() {
        ...
        // server_info.name 应保持 build_env 默认值（memory-center-mcp）
        assert!(
            info.server_info.name.contains("memory-center"),
            "server_info.name 应含 memory-center，实际: {}",
