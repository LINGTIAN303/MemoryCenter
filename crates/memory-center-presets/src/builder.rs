...
use memory_center_scenarios::{Scenario, ScenarioProfile, SummaryFocus};
use memory_center_skills::SkillProfile;
...
fn generate_usage_protocol(
    ...
    let instructions = format!(
        "你正在 {agent_name} + {scenario_lower} 场景下工作，已接入 MemoryCenter 记忆库。\n\
         \n\
         ## 记忆协议（无需用户提示，主动执行）\n\
         \n\
         1. **会话开始**：第一步调用 `mcp_MemoryCenter.prompt(session_id)` 获取历史记忆摘要，拼接到 system prompt\n\
         2. **{threshold_desc}**：调用 `mcp_MemoryCenter.archive(session_id, turns_json)` 归档完整上下文\n\
         3. **用户提到过去事件**：先调 `mcp_MemoryCenter.semantic_search(query, session_id)` 检索，再回复\n\
         4. **用户陈述与记忆可能矛盾**：先调 `mcp_MemoryCenter.detect_conflicts(session_id, statement)` 检测\n\
         \n\
         ## session_id 约定\n\
         {session_id_pattern}\n\
         \n\
...
pub fn scenario_to_str(scenario: &memory_center_scenarios::Scenario) -> String {
...
pub fn scenario_from_str(s: &str) -> memory_center_scenarios::Scenario {
    let lower = s.to_lowercase();
    if let Some(custom_val) = lower.strip_prefix("custom:") {
        return memory_center_scenarios::Scenario::Custom(custom_val.to_string());
    }
    match lower.as_str() {
        "coding" => memory_center_scenarios::Scenario::Coding,
        "writing" => memory_center_scenarios::Scenario::Writing,
        "research" => memory_center_scenarios::Scenario::Research,
        "daily" => memory_center_scenarios::Scenario::Daily,
...
///
/// 公共函数，供 `memory-center-server` 的 archive handler / build_preset 端点、
/// `memory-center-mcp` 的 archive tool / preset_build tool 共用。
///
/// ## 参数
///
...
pub fn build_from_strings(
    ...
    if let Some(agent_str) = agent {
        let family = memory_center_agents::AgentFamily::from_str(agent_str)
            .unwrap_or_else(|| memory_center_agents::AgentFamily::Custom(agent_str.to_string()));
    ...
    if let Some(model_str) = model {
        match memory_center_models::ModelRegistry::find(model_str) {
            Some(variant) => {
...
mod tests {
    ...
    fn test_model_archive_threshold() {
        use memory_center_models::variant::ArchiveStrategy;
    ...
    fn test_invalid_scenario_returns_error() {
        use memory_center_scenarios::{RetrievalStrategy, ScoreWeights};
    ...
    fn test_usage_protocol_generated_for_claude_code() {
        ...
        assert!(protocol.instructions.contains("mcp_MemoryCenter.prompt"));
        assert!(protocol.instructions.contains("mcp_MemoryCenter.archive"));
