//! # MemoryCenter Python 绑定
//!
//! 使用 PyO3 将 [`memory_center_core`] 的能力暴露为 Python 原生扩展模块。
//!
//! ## 架构
//!
...
//! - **上下文管理器**：支持 `with MemoryCenter(...) as hp:` 用法
//!
//! ## 使用示例
//!
//! ```python
...
//!
//! with MemoryCenter("./data", "session-1", project_id="proj-a") as hp:
//!     # 归档
//!     summary = hp.archive([
...
/// MemoryCenter Python 扩展模块
///
/// 模块名 `memory_center_python`（与 Cargo.toml lib.name 一致）
#[pymodule]
mod memory_center_python {
    use super::*;
    ...
    fn supported_models() -> Vec<String> {
        memory_center_models::ModelRegistry::all_variants()
...
/// 一个实例对应一个会话（与 FFI 层 MemoryCenterHandle 一致）。
///
/// Python 用法：
/// ```python
/// hp = MemoryCenter("./data", "session-1", project_id="proj-a")
/// summary = hp.archive(turns)
/// hp.close()  # 或用 with 上下文管理器
/// ```
#[pyclass(name = "MemoryCenter")]
struct MemoryCenter {
    /// 存储根目录
...
///
/// 环境变量前缀 `MEMORY_CENTER_GENERATOR_`，与 server/mcp 一致：
/// - `MEMORY_CENTER_GENERATOR_API_URL`：LLM API 地址（必填，未设置则返回 None）
/// - `MEMORY_CENTER_GENERATOR_API_KEY`：API Key（可选）
/// - `MEMORY_CENTER_GENERATOR_MODEL`：模型名（可选，默认 gpt-4o-mini）
/// - `MEMORY_CENTER_GENERATOR_MAX_TOKENS`：最大 token 数（可选，默认 1024）
///
...
///
/// 与 `memory-center-server` 的 `build_combined_from_request` 和
/// `memory-center-mcp` 的 archive tool 复用同一套 `build_from_strings` 解析逻辑。
///
/// ## 支持的 dict 字段（全部可选）
///
...
fn build_combined_from_preset(
    preset_obj: Py<PyAny>,
) -> PyResult<memory_center_presets::CombinedProfile> {
...
impl MemoryCenter {
    ...
    /// 若需 LLM 摘要，请使用 [`MemoryCenter::from_env`] 类方法。
    #[new]
    ...
    /// 从环境变量构建 MemoryCenter 实例（v2.23）
    ///
    /// 在 [`MemoryCenter::new`] 基础上，从环境变量读取 LLM 配置并注入摘要生成器。
    /// archive/compaction 将使用 LLM 生成的结构化摘要（失败时降级为启发式）。
    ///
    /// ## 环境变量（前缀 `MEMORY_CENTER_GENERATOR_`）
    ///
    /// - `MEMORY_CENTER_GENERATOR_API_URL`：LLM API 地址（必填，未设置则不注入 LLM）
    /// - `MEMORY_CENTER_GENERATOR_API_KEY`：API Key（可选）
    /// - `MEMORY_CENTER_GENERATOR_MODEL`：模型名（可选，默认 gpt-4o-mini）
    /// - `MEMORY_CENTER_GENERATOR_MAX_TOKENS`：最大 token 数（可选，默认 1024）
    ///
    ...
    /// ## 返回
    ///
    /// MemoryCenter 实例（含 LLM 摘要生成器，若环境变量未配置则降级为启发式）
    ///
    /// ## Python 用法
    ///
    /// ```python
    /// import os
    /// os.environ["MEMORY_CENTER_GENERATOR_API_URL"] = "https://api.openai.com/v1"
    /// os.environ["MEMORY_CENTER_GENERATOR_API_KEY"] = "sk-xxx"
    /// os.environ["MEMORY_CENTER_GENERATOR_MODEL"] = "gpt-4o-mini"
    ///
    /// from memory_center_python import MemoryCenter
    /// hp = MemoryCenter.from_env("./data", "session-1")
    ...
    fn archive(
        ...
        let summary = memory_center_core::retrieve::SummaryView::from(&hook);
...
///
/// 未匹配的字符串返回 None。
fn window_scheme_from_str(s: &str) -> Option<memory_center_windows::WindowProfile> {
    use memory_center_windows::WindowProfile;
...
/// Python 用法：
/// ```python
/// from memory_center_python import PresetBuilder
///
...
struct PyPresetBuilder {
    inner: memory_center_presets::PresetBuilder,
}
...
impl PyPresetBuilder {
    ...
    fn new() -> Self {
        Self {
            inner: memory_center_presets::PresetBuilder::new(),
        }
    }
    ...
    fn with_agent(&mut self, agent: String) {
        let family = memory_center_agents::AgentFamily::from_str(&agent)
            .unwrap_or_else(|| memory_center_agents::AgentFamily::Custom(agent.clone()));
    ...
    fn with_scenario(&mut self, scenario: String) {
        ...
        let sc = memory_center_presets::scenario_from_str(&scenario);
    ...
    fn with_model(&mut self, model: String) -> PyResult<()> {
        let variant = memory_center_models::ModelRegistry::find(&model).ok_or_else(|| {
            PyValueError::new_err(format!(
                "未找到型号: {}（用 memory_center_python.supported_models() 查询可用值）",
                model
            ))
        })?;
    ...
    fn with_skill(&mut self, skill: String) {
        let builtin = memory_center_skills::BuiltinSkill::from_str(&skill)
            .unwrap_or_else(|| memory_center_skills::BuiltinSkill::Custom(skill.clone()));
        let profile = memory_center_skills::SkillProfile::new(builtin);
    ...
    fn with_skills(&mut self, skills: Vec<String>) {
        let profiles: Vec<memory_center_skills::SkillProfile> = skills
            .into_iter()
            .map(|s| {
                let builtin = memory_center_skills::BuiltinSkill::from_str(&s)
                    .unwrap_or_else(|| memory_center_skills::BuiltinSkill::Custom(s.clone()));
                memory_center_skills::SkillProfile::new(builtin)
            })
            .collect();
...
mod tests {
    ...
    ///
    /// v2.29：本地 scenario_from_str 已删除，改用 memory_center_presets::scenario_from_str
    #[test]
    fn test_scenario_from_str_case_insensitive() {
        use memory_center_scenarios::Scenario;

        assert_eq!(memory_center_presets::scenario_from_str("coding"), Scenario::Coding);
        assert_eq!(memory_center_presets::scenario_from_str("Coding"), Scenario::Coding);
        assert_eq!(memory_center_presets::scenario_from_str("CODING"), Scenario::Coding);
        assert_eq!(memory_center_presets::scenario_from_str("writing"), Scenario::Writing);
        assert_eq!(memory_center_presets::scenario_from_str("research"), Scenario::Research);
        assert_eq!(memory_center_presets::scenario_from_str("daily"), Scenario::Daily);
        assert_eq!(memory_center_presets::scenario_from_str("finance"), Scenario::Finance);
        assert_eq!(memory_center_presets::scenario_from_str("design"), Scenario::Design);
    }
    ...
    fn test_scenario_from_str_office_aliases() {
        ...
        assert_eq!(memory_center_presets::scenario_from_str("officework"), Scenario::OfficeWork);
        assert_eq!(memory_center_presets::scenario_from_str("office"), Scenario::OfficeWork);
        assert_eq!(memory_center_presets::scenario_from_str("work"), Scenario::OfficeWork);
        assert_eq!(memory_center_presets::scenario_from_str("Office"), Scenario::OfficeWork);
        assert_eq!(memory_center_presets::scenario_from_str("WORK"), Scenario::OfficeWork);
    }
    ...
    fn test_scenario_from_str_custom_fallback() {
        ...
        match memory_center_presets::scenario_from_str("游戏场景") {
            Scenario::Custom(s) => assert_eq!(s, "游戏场景"),
            other => panic!("期望 Custom，实际 {:?}", other),
        }

        match memory_center_presets::scenario_from_str("unknown") {
    ...
    fn test_supported_agents_count() {
        ...
        assert_eq!(memory_center_agents::AgentFamily::all_builtin().len(), 11);
    }
    ...
    fn test_supported_skills_count() {
        assert_eq!(memory_center_skills::BuiltinSkill::all_builtin().len(), 15);
    }
    ...
    fn test_preset_builder_with_model_rust_side() {
        let variant = memory_center_models::ModelRegistry::find("claude-opus-4.8")
            .expect("claude-opus-4.8 应存在");
        let combined = memory_center_presets::PresetBuilder::new()
            .with_model(variant)
            .build()
            .expect("build 失败");
    ...
    fn test_preset_builder_with_skills_rust_side() {
        ...
        let combined2 = memory_center_presets::PresetBuilder::new()
            .with_skills(batch)
            .build()
            .expect("build 失败");
    ...
    /// v2.29：验证 build_from_strings 与 MemoryCenter.archive(preset=...) 等价路径
    ///
    ...
    fn test_build_from_strings_errors() {
        // 未找到型号
        let err = memory_center_presets::build_from_strings(
    ...
    async fn test_archive_with_preset_threshold_applied() {
        ...
        // 构造 ArchiveConfig（与 MemoryCenter.archive 等价逻辑）
    ...
    fn test_preset_builder_rust_side() {
        ...
        let builder = memory_center_presets::PresetBuilder::new()
            .with_agent(memory_center_agents::AgentProfile::claude_code())
            .with_scenario(memory_center_scenarios::ScenarioProfile::from_scenario(
                memory_center_scenarios::Scenario::Coding,
            ))
            .with_user_archive_threshold(450_000);
    ...
    fn test_build_summary_generator_from_env() {
        ...
        std::env::remove_var("MEMORY_CENTER_GENERATOR_API_URL");
        std::env::remove_var("MEMORY_CENTER_GENERATOR_API_KEY");
        assert!(
        ...
        std::env::set_var("MEMORY_CENTER_GENERATOR_API_URL", "https://api.openai.com/v1");
        std::env::set_var("MEMORY_CENTER_GENERATOR_API_KEY", "sk-test-key");
    ...
    async fn test_archive_with_summary_generator_injection() {
        ...
        impl SummaryGenerator for MockGen {
            async fn generate_summary(&self, _file: &MemoryFile) -> memory_center_core::Result<Summary> {
                Ok(Summary {
