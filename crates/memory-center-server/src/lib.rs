//! # MemoryCenter HTTP 服务库
//!
//! 将 [`memory_center_core`] 的核心能力暴露为 REST API，
...
// v2.18 批次2：搜索模块下沉到 MemoryCenter-search crate
...
impl Default for Config {
    fn default() -> Self {
        Self {
            host: std::env::var("MEMORY_CENTER_HOST").unwrap_or_else(|_| "127.0.0.1".to_string()),
            port: std::env::var("MEMORY_CENTER_PORT")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(8765),
            storage_root: PathBuf::from(
                std::env::var("MEMORY_CENTER_ROOT").unwrap_or_else(|_| "./data".to_string()),
            ),
        }
    }
}
...
pub struct AppState {
    ...
    pub conflict_detector:
        Option<std::sync::Arc<dyn memory_center_core::conflict::ConflictDetector>>,
    ...
    pub summary_generator:
        Option<std::sync::Arc<dyn memory_center_core::generate::SummaryGenerator>>,
    /// 可选的场景识别器（v2.33）
    ///
...
/// 创建路由
///
/// v2.24：新增 API Key 鉴权中间件（环境变量 `MEMORY_CENTER_API_KEY` 驱动）
