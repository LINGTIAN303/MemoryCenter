//! MemoryCenterCore 端到端 API 测试
//!
//! 验证 MemoryCenterCore JS API 的 4 个核心方法：
...
use memory_center_core_logic::model::{IndexDocument, MemoryFile, MessageContent, MessageTurn, Tag};
...
/// 将 Vec<MessageTurn> 序列化为 JsValue（供 MemoryCenterCore.archive 接收）
...
async fn test_memory_center_core_archive_and_retrieve() {
    ...
    let core = MemoryCenterCore::with_memory_storage(storage);
...
#[wasm_bindgen_test]
async fn test_memory_center_core_read_memory() {
    let storage = MemoryStorage::new();
    let core = MemoryCenterCore::with_memory_storage(storage);
...
#[wasm_bindgen_test]
async fn test_memory_center_core_read_index() {
    let storage = MemoryStorage::new();
    let core = MemoryCenterCore::with_memory_storage(storage);
...
#[wasm_bindgen_test]
async fn test_memory_center_core_list_memories_empty() {
    let storage = MemoryStorage::new();
    let core = MemoryCenterCore::with_memory_storage(storage);
...
#[wasm_bindgen_test]
async fn test_memory_center_core_invalid_period() {
    let storage = MemoryStorage::new();
    let core = MemoryCenterCore::with_memory_storage(storage);
