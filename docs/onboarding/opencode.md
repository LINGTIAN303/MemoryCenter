...
- **Windows**: `~/.config/opencode/opencode.jsonc`（即 `%USERPROFILE%\.config\opencode\opencode.jsonc`）
...
| Windows | `~/.local/share/opencode/opencode.db`（即 `%USERPROFILE%\.local\share\opencode\opencode.db`） |
