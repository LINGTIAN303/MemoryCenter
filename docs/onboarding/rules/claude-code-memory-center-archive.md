# MemoryCenter 记忆归档触发规则（Claude Code）
...
## session_id 约定
claude-code-{项目名}-{日期}，如 claude-code-myapp-20260705

## Claude Code 特有提示
在 Claude Code /compact 前，先调用 MemoryCenter.archive 归档当前对话，避免压缩丢失原始内容。
...
> 通过识别 Claude Code 客户端注入的固定标记消息，触发强制校准流程。

### 1. 触发条件：识别 Marker Message

当你在系统消息中看到以下**固定文本**时，表明上下文刚被 Claude Code 客户端压缩：
...
**关键判断**：
- 该文本是 Claude Code 编码层固定返回，100% 可靠
- 看到"lost its context"即视为压缩触发信号
- 不要将其当作普通的会话开场白

> **注意**：上述 marker text 为 Trae 客户端的压缩标记。Claude Code 使用 `/compact` 命令触发压缩，标记文本可能不同，核心流程一致但需根据实际标记调整。
...
- MemoryCenter 记忆是**一手源数据**，优先级高于 Claude Code 生成的 Summary

#### 步骤 3：交叉校准 Current Work（对应 Summary 第8章节）
比对 Claude Code Summary 的"Current Work"章节与 MemoryCenter 记忆：
...
> 通过 Pending todos（Claude Code 注入的状态）校准 Next Step（Claude Code 生成的建议）。
...
- Claude Code 生成的 Next Step 不知道"你已经在做了"，可能给出错误建议
...
> 主动写入 Claude Code 客户端的 project_memory.md，影响下次会话的第7层注入。
...
#### 步骤 2：用 Write 工具写入 Claude Code 的 project_memory.md

将 `full_content` 写入 Claude Code 客户端的 memory 文件夹：
- Claude Code: `~/.claude/memory/projects/<project>/project_memory.md`
...
- **返回的 full_content 必须完整写入 Claude Code 文件**：不能只写部分
