# Hippocampus 评测报告 V2.3

> 生成时间：2026-07-03
> 评测目的：验证 Hippocampus 记忆库对 Agent 长时记忆能力的提升效果
...
| 条件对照 | baseline（裸考，完整对话历史直接拼入 messages）vs hippo（Hippocampus 归档 + /prompt + retrieve 完整内容注入 system prompt）|
...
| 模型 | baseline | hippo | 差异 |
|------|----------|-------|------|
| sensenova | 0.7333 | 0.7333 | 持平 |
| step | 0.7333 | 0.7333 | 持平 |

**能力分布变化**（hippo vs baseline，5 题/能力）：
...
- hippo 改变了能力分布：sensenova 在 preference 提升，step 在 multi-session 提升
- 单 session 能力（assistant/user）两组都保持 1.0 满分，hippo 无负面影响

### 2.2 LoCoMo（90 题 × 4 组，F1 算法评分）

| 模型 | baseline F1 | hippo F1 | 提升 |
|------|-------------|----------|------|
...
| category | 类型 | sensenova base→hippo | step base→hippo |
|----------|------|----------------------|-----------------|
...
| sample | 规模 | sensenova base→hippo | step base→hippo |
|--------|------|----------------------|-----------------|
...
### 3.1 LoCoMo 是 Hippo 优势的有力证据

LoCoMo 用纯算法 F1 评分（无 judge 主观性），hippo 在两个模型上都显著提升 overall F1：
- sensenova +41.4%
- step +21.7%

### 3.2 hippo 对多跳推理帮助最大

LoCoMo category_1（多跳推理）两个模型都显著提升（+25% ~ +48.7%），证明 Hippocampus 的归档 + retrieve 机制让模型能跨 session 关联信息。

### 3.3 大规模对话 hippo 优势更明显

conv-41（663 turns，最大规模）hippo 提升最显著：sensenova +67%、step +94%。
小规模 conv-30（369 turns）hippo 优势减弱甚至略降，可能因 baseline 短上下文已能覆盖。

### 3.4 LongMemEval flash judge 抹平差异

DeepSeek-V4-flash 比 V4-pro 更宽松，sensenova baseline 从 0.4667 升至 0.7333，天花板效应掩盖 hippo 优势。
...
| /prompt 只返回摘要标题 | 无 retrieve 完整内容入口 | 新增 hippo_retrieve_all_content() 函数 |
| LoCoMo evaluate_sample tuple 未解包 | locomo_run_hippo 返回 tuple 但调用方未解包 | 改为 hippo_summary, hippo_content = ... |
| call_llm 无 timeout | LoCoMo baseline 大 messages 无限挂起 | 加 timeout=180 |

### 4.3 资源占用

- hippocampus-server：本地运行，127.0.0.1:8765
- 总耗时：LongMemEval 约 28 分钟 + LoCoMo 约 80 分钟（并行执行）
- API 调用：LongMemEval ~120 次 + LoCoMo ~360 次

## 5. 结论

Hippocampus 记忆库在 LoCoMo 长时记忆基准上验证有效：
- **纯算法 F1 评分下，两个模型 overall F1 都显著提升（+21.7% ~ +41.4%）**
- **多跳推理能力提升最显著（+25% ~ +48.7%）**
- **大规模对话场景 hippo 优势更明显（conv-41 step +94%）**
